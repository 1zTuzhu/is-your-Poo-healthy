 ...
 * For example:
 *
 * dispatching a known click 馃憥
 * target.dispatchEvent(new Event(鈥榗lick鈥?)
 *
 * dispatching a custom type within a non-discrete event 馃憥
 * onScroll={(event) => event.target.dispatchEvent(new CustomEvent(鈥榗ustomType鈥?)}
 *
 * dispatching a custom type within a `discrete` event 馃憤
 * onPointerDown={(event) => dispatchDiscreteCustomEvent(event.target, new CustomEvent(鈥榗ustomType鈥?)}
 *
