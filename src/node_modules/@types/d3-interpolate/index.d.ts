...
export interface ColorGammaInterpolationFactory extends Function {
     ...
     * See Eric Brasseur鈥檚 article, [Gamma error in picture scaling](https://web.archive.org/web/20160112115812/http://www.4p8.com/eric.brasseur/gamma.html), for more on gamma correction.
     */
 ...
 * Then, for the given parameter `t`, the template鈥檚 embedded interpolators are evaluated. The updated array template is then returned.
 *
 ...
 * based on [鈥淪mooth and efficient zooming and panning鈥漖(http://www.win.tue.nl/~vanwijk/zoompan.pdf).
...
/**
 * Returns a uniform nonrational B-spline interpolator through the specified array of colors, which are converted to RGB color space.
 * The control points are implicitly repeated such that the resulting spline has cyclical C虏 continuity when repeated around `t` in [0,1];
 ...
 * they will be converted to HSL using `d3.hsl`. If either color鈥檚 hue or saturation is NaN, the opposing color鈥檚 channel value is used.
 ...
 * they will be converted to HCL using `d3.hcl`. If either color鈥檚 hue or chroma is NaN, the opposing color鈥檚 channel value is used.
 * The shortest path between hues is used. The return value of the interpolator is an RGB string.
 */
 ...
 * If either color鈥檚 hue or saturation is NaN, the opposing color鈥檚 channel value is used. The shortest path between hues is used. The return value of the interpolator is an RGB string.
 */
...
/**
 * Returns a uniform nonrational B-spline interpolator through the specified array of `values`, which must be numbers.
 * The control points are implicitly repeated such that the resulting one-dimensional spline has cyclical C虏 continuity when repeated around `t` in [0,1].
